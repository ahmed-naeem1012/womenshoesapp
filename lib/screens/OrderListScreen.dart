  ...
  Future fetchOrderData() async {
    ...
    await getOrders().then((res) {
      ...
      setState(() {
        ...
        log("response ${mOrderDetails.length}");
        mOrderModel = mOrderDetails.map((model) => OrderResponseModel.fromJson(model)).toList();
        log("orderDetail: ${mOrderModel!.last}");
      });
    }).catchError((error) {
      if (!mounted) return;
      log(error);
                          ...
                          if (mOrderModel![i].lineItems!.length > 1)
                            Column(
                              ...
                              children: [
                                ...
                                4.height,
                                Text(appLocalization.translate("lbl_more_items").toString(), style: secondaryTextStyle(color: primaryColor!.withOpacity(0.5))),
                              ],
                            )
                          else
                            Text(mOrderModel![i].lineItems![0].name.validate(), maxLines: 2, overflow: TextOverflow.ellipsis, style:primaryTextStyle())
        ...
        body: mInternetConnection(
          Stack(
            children: [
              mOrderModel!.isNotEmpty
                ...
                mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      ...
                      Text(appLocalization.translate("msg_empty_order")!, style: primaryTextStyle(size: 18), textAlign: TextAlign.center).paddingOnly(left: 20, right: 20),
                      4.height,
                      Text(appLocalization.translate("msg_info_empty_order")!, style: secondaryTextStyle(size: 14), textAlign: TextAlign.center).paddingOnly(left: 20, right: 20),
                      30.height,
                      Container(
                        width: context.width(),
                        child: AppButton(
              ...
              Text(mErrorMsg, style: primaryTextStyle(), textAlign: TextAlign.center).visible(!appStore.isLoading!).center().visible(mErrorMsg.isNotEmpty),
            ],
          ),
        ),
      ),
