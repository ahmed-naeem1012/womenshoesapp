...
class _OfferScreenState extends State<OfferScreen> {
  ...
  Future fetchOfferData() async {
    ...
    await offerProductList().then((res) {
      ...
      setState(() {
        ...
        mOfferProduct = list.map((model) => ProductResponse.fromJson(model)).toList();
  ...
  Widget build(BuildContext context) {
      ...
      return StaggeredGridView.countBuilder(
        ...
        itemBuilder: (context, i) {
          if (builderResponse.productdetailview!.layout == 'productDetail1')
            return ProductComponent1(mProductModel: product[i], width: context.width() * 0.4).paddingOnly(top: 8);
          else if (builderResponse.productdetailview!.layout == 'productDetail2')
            return ProductComponent2(mProductModel: product[i], width: context.width() * 0.4).paddingOnly(top: 8);
          else
            return ProductComponent3(mProductModel: product[i], width: context.width() * 0.4).paddingOnly(top: 8);
        },
    ...
    Widget dealOfTheDay() {
      return Column(
        children: [
          availableOfferAndDeal(mOfferProduct).visible(mOfferProduct.isNotEmpty),
        ],
      );
    }

    return Scaffold(
      appBar: mTop(context, appLocalization!.translate('lbl_offer_zone'), showBack: true) as PreferredSizeWidget?,
