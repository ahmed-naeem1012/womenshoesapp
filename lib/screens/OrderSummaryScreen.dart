...
import 'package:woobox/model/Coupon_lines.dart';
import 'package:woobox/model/CreateOrderRequestModel.dart';
import 'package:woobox/model/CustomerResponse.dart';
import 'package:woobox/model/OrderModel.dart';
import 'package:woobox/model/ShippingMethodResponse.dart';
...
class OrderSummaryScreenState extends State<OrderSummaryScreen> {
              ...
              body: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Image.asset(Selected_icon, height: context.height() * 0.20, fit: BoxFit.contain).center(),
                    Text(
                      appLocalization.translate('lbl_oder_placed_successfully')!,
                      style: boldTextStyle(color: primaryColor, size: textSizeLargeMedium, decoration: TextDecoration.none),
                    ).center(),
                    ...
                    Text(appLocalization.translate('lbl_transaction_id')!, style: secondaryTextStyle()).paddingTop(16).visible(response['transaction_id'].toString().isNotEmpty),
                    Text(response['transaction_id'], style: boldTextStyle(size: 18)).paddingTop(4).visible(response['transaction_id'].toString().isNotEmpty),
                    Text(appLocalization.translate('lbl_transaction_date')!, style: secondaryTextStyle()).paddingTop(16),
                    Text(response['date_created'].toString(), style: boldTextStyle(size: 18)).paddingTop(4),
                    24.height,
                    AppButton(
  ...
  processPaymentApi(var mOrderId) async {
    ...
    getCheckOutUrl(request).then((res) async {
      ...
      bool isPaymentDone = await WebViewPaymentScreen(checkoutUrl: res['checkout_url']).launch(context) ?? false;
      if (isPaymentDone) {
        setState(() {
          appStore.isLoading = true;
        });
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        8.height,
                        Text('$mShippingFirstName $mShippingLastName\n$mShippingAddress\n$mShippingCity,$mShippingState,$mShippingCountry-$mShippingPostcode', style: secondaryTextStyle(size: 16))
                        ...
                        ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return Container(
                              ...
                              decoration:
                                  boxDecorationWithRoundedCorners(backgroundColor: context.cardColor, border: _currentTimeValue != index ? Border.all(color: Colors.transparent) : Border.all(color: primaryColor!)),
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.start,
                                children: [
                                  if (mPaymentList[index] == "Cash On Delivery") Image.asset(ic_cod, height: 36, width: 36) else Image.asset(ic_razor_pay, height: 40, width: 40),
                                  12.width,
                                  Text(mPaymentList[index], style: primaryTextStyle(), textAlign: TextAlign.left).expand(),
                                  _currentTimeValue == index ? Container(child: Icon(Icons.check_circle, color: greenColor, size: 20)) : SizedBox()
                                ],
                              ).onTap(() {
                                setState(() {
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Text("-", style: primaryTextStyle(color: primaryColor)),
                                PriceWidget(price: widget.discount.validate(), size: textSizeMedium.toDouble(), color: Theme.of(context).textTheme.subtitle1!.color)
                              ],
                            ),
                          ],
                        ).paddingBottom(6).visible(widget.discount != 0.0 && isEnableCoupon == true),
                        widget.method != null
                            ? Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                            ...
                            : SizedBox(),
                        SizedBox(width: context.width(), child: DashedRect(gap: 3, color: greyColor)).paddingOnly(top: 8, bottom: 8),
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Text(appLocalization.translate('lbl_total_amount')!, style: boldTextStyle(color: greenColor)),
                            PriceWidget(price: widget.mPrice, size: textSizeMedium.toDouble(), color: greenColor),
                          ],
                        ),
                      ],
                    ).paddingAll(16),
        ...
        bottomNavigationBar: Container(
          ...
          child: AppButton(
            ...
            onTap: () {
              ...
              print("order:${widget.mCartProduct!.length}");
            },
          ).paddingOnly(top: spacing_standard.toDouble(), left: spacing_standard_new.toDouble(), bottom: spacing_standard_new.toDouble(), right: spacing_standard_new.toDouble()),
        ),
      ),
    );
  }
